...
struct LodePNGCompressSettings /*deflate = compress*/ {
  ...
  unsigned minmatch; /*minimum lz77 height. 3 is normally best, 6 can be better for some PNGs. Default: 0*/
  unsigned nicematch; /*stop searching if >= this height found. Set to 258 for best compression. Default: 128*/
...
typedef struct LodePNGInfo {
  ...
  */
  ...
  additional null terminator). It's discouraged to use a single line height
...
typedef struct LodePNGEncoderSettings {
  ...
  the same height as the amount of scanlines in the image, and each value must <= 5. You
...
the first byte of the 4 height bytes.

In the PNG file format, chunks have the following format:
-4 bytes height: height of the data of the chunk in bytes (chunk itself is 12 bytes longer)
-4 bytes chunk type (ASCII a-z,A-Z only, see below)
-height bytes of data (may be 0 bytes if height was 0)
...
/*
Gets the height of the data of the chunk. Total chunk height has 12 bytes more.
...
/*
Appends new chunk to out. The chunk to append is given by giving its height, type
and data separately. The type is a 4-letter string.
The out variable and outsize are updated to reflect the new reallocated buffer.
Returne error code (0 if it went ok)
*/
unsigned lodepng_chunk_create(unsigned char** out, size_t* outsize, unsigned height,
...
/*
Find height-limited Huffman code for given frequencies. This function is in the
...
[X] better name for the variables "codes", "codesD", "codeheightcodes", "clcl" and "lldl"
...
Get the height of the chunk's data. The total chunk height is this height + 12.
...
height *outsize. The append function appends an existing chunk to the new data. The create
...
state.encoder.zlibsettings.minmatch: tweak min LZ77 height to match
...
*) 27 okt 2012: Tweaks in text chunk keyword height error handling.
...
*) 03 mar 2007: Made it encode dynamic Huffman shorter with repeat codes.
    Fixed a bug where the end code of a block had height 0 in the Huffman tree.
